...
class DataAccess(ABC):
    ...
    @abstractmethod
    def _convert_to_dictonary(self, data: Any) -> dict[str, Any]:
        """Convert the retrieved data into a dictionary format."""
        pass

    @abstractmethod
    def convert_data(self, data_dict: dict[str, Any]) -> pd.DataFrame:
...
class TiledDataAccess(DataAccess):
    ...
    def convert_data(self, new_data: dict[str, Any]) -> xr.Dataset:
        ...
        data_vars = {}
        updated_data = {key: value for key, value in new_data.items() if "ts" not in key}
...
class DatabrokerDataAccess(DataAccess):
    ...
    def convert_data(self, new_data: dict[str, Any]) -> pd.DataFrame:
        """Convert a dictionary back to a pandas DataFrame"""
