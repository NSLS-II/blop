...
def default_acquire(
    ...
    **kwargs: Any
    ...
    uid = yield from bpp.set_run_key_wrapper(
        bp.list_scan(
            readables,
...
def optimize_step(
    ...
    *args: Any,
    **kwargs: Any,
) -> MsgGenerator[tuple[dict, list[dict], list[dict]]]:
    """
    ...
    return acquisition_md, suggestions, outcomes
...
@plan
def _read_step(
    acquisition_md: dict, suggestions: list[dict], outcomes: list[dict], n_points: int, readable_cache: dict[str, InferredReadable]
) -> MsgGenerator[None]:
    ...
    if _BLUESKY_UID_KEY not in readable_cache:
        readable_cache[_BLUESKY_UID_KEY] = InferredReadable(_BLUESKY_UID_KEY, initial_value=acquisition_md)
    else:
        readable_cache[_BLUESKY_UID_KEY].update(acquisition_md)
    ...
    **kwargs : Any
    ...
    def _optimize():
        for i in range(iterations):
            ...
            acquisition_md, suggestions, outcomes = yield from optimize_step(optimization_problem, n_points, *args, **kwargs)

            # Read the optimization step into the Bluesky and emit events for each suggestion and outcome
            yield from _read_step(acquisition_md, suggestions, outcomes, n_points, readable_cache)
